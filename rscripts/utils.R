...
read_file_and_parse_dp_columns <- function(f, loss_group){
  df = read.csv(f)
  df$uid = f
...
add_dp_str_column <- function(df){
  df$dp_str <- do.call(paste0, list("DP", df$dp, 
                                         "S", df$S, 
                                         "z", df$z, "sigma", df$sigma))
  return(df)
}
...
read_data <- function(dirname, loss_group){
  ...
  for (f in list.files(dirname, full.names = TRUE)){
    tmp[[f]] = read_file_and_parse_dp_columns(f, loss_group)
  }
  ...
  results <- add_dp_str_column(results)
  return(results)
}
...
read_data_2 <- function(dirname, loss_group){
  message("[INFO] this function needs updating! It has dp info hard-coded.")
  tmp = list()
...
read_convergence_data <- function(dirname, loss_group){
  ...
  results = dplyr::bind_rows(tmp) %>%
    select(c("Value", "alpha", "dp", "S", "z", "sigma", "test_class", "Step"))
  results <- add_dp_str_column(results)
  return(results)
}
