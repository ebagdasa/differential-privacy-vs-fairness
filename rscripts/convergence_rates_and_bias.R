...
p1 <- mnist_convergence %>%
  dplyr::filter(as.integer(dp_str) < 3) %>%
  ggplot(aes(x=Step, y=Value, group=factor(alpha))) +
  geom_line(aes(col=alpha)) +
  facet_grid(rows = vars(factor(test_class)),
             cols = vars(dp_str),
...
mmnist_subgroup_loss_plot <- mnist_convergence %>%
  dplyr::filter(Step==119, as.integer(dp_str) < 3) %>%
...
# mmnist_arranged <- ggarrange(p2, p1, nrow=1, ncol=2, common.legend = TRUE, legend="bottom")
# annotate_figure(mmnist_arranged, top=text_grob("MMNIST", size=24))
# ggsave("./convergence_loss_mnist_subset.pdf", device="pdf", width=8, height=5.25)
